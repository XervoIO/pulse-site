var Ball = pulse.Sprite.extend({
   init: function(args) {
      ...
      args = args || {};
      args.src = '/demos/ball-world/img/ball.png';
   ...
   update: function(elapsedMS) {
      ...
      if(newX + (this.size.width / 2) >= 600) {
         newX = 600 - this.size.width / 2;
         this.velocity.x *= -1;
      }

      if(newY + (this.size.height / 2) >= 400) {
         newY = 400 - this.size.height / 2;
         this.velocity.y *= -1;
      }
